...
public class IDS {
     ...
     */
    public IDS() {
        IDSAdapter adapter = new IDSAdapter();
        NanoSatMOConnectorImpl connector = new NanoSatMOConnectorImpl();
        connector.init(adapter);

        // Initialize application's NMF consumers (consuming the supervisor and camera acquisitor app)
        SpaceMOApdapterImpl supervisorSMA = SpaceMOApdapterImpl.forNMFSupervisor(connector
            .readCentralDirectoryServiceURI());
        SpaceMOApdapterImpl spaceAppSMA = SpaceMOApdapterImpl.forNMFApp(connector
            .readCentralDirectoryServiceURI(), "camera-acquisitor-system");

        // Set up all connections and start monitoring.
